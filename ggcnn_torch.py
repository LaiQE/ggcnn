...
from dougsm_timeit import TimeIt


class GGCNNTorch(object):
        ...
        if return_mask:
            with TimeIt('6'):
                ...
                depth_nan_mask = cv2.resize(
                    depth_nan_mask, (out_size, out_size), cv2.INTER_NEAREST)
    ...
    def predict(self, depth, process_depth=True, crop_size=300, out_size=300, depth_nan_mask=None, crop_y_offset=0, filters=(2.0, 1.0, 1.0)):
        if process_depth:
            depth, depth_nan_mask = self.process_depth_image(
                depth, crop_size, out_size=out_size, return_mask=True, crop_y_offset=crop_y_offset)

        # Inference
        depth = np.clip((depth - depth.mean()), -1, 1)
        depthT = torch.from_numpy(depth.reshape(
            1, 1, out_size, out_size).astype(np.float32)).to(self.device)
        with torch.no_grad():
